...
export const RentalPeriod: React.FC<RentalPeriodProps> = ({
  ...
  const [localReturnDate, setLocalReturnDate] = useState<Dayjs | null>(returnDate);
  ...
  const disabledDateFinish: RangePickerProps['disabledDate'] = (current) => {
    if (!localStartDate) return true;
    return current && current < localStartDate.startOf('day');
  };

  useEffect(() => {
    if (!localStartDate) return;

    if (localStartDate && localReturnDate) {
      const exactDiffHours = localReturnDate.diff(localStartDate, 'hour', true);
      let totalDays = Math.max(0, Math.ceil(exactDiffHours / 24));
      if (totalDays < 3) {
        const adjustedEnd = localStartDate.add(3, 'day');
        setLocalReturnDate(adjustedEnd);
        onReturnDateChange?.(adjustedEnd);
        totalDays = 3;
      }
  ...
  }, [localStartDate, localReturnDate]);

  const rentDate = `${localStartDate?.format('DD.MM.YYYY')} ${localStartTime || defaultTimeValue} – ${localReturnDate?.format('DD.MM.YYYY')} ${localReturnTime || defaultTimeValue}`;
  ...
  return (
    <div className='w-full bg-[#284b63] rounded-2xl px-6 py-7 mt-6 lg:mt-0 relative z-10 lg:p-9 lg:rounded-[32px]'>
      {showContactForm && closeModal && (
        <div className='absolute top-6 right-6 cursor-pointer' onClick={closeModal}>
          <CloseIcon />
        </div>
      )}
      ...
      <div className='flex flex-col gap-2 lg:flex-row lg:gap-[10px]'>
        <div className='flex'>
          ...
          <CustomSelect
            ...
            value={localStartTime || defaultTimeValue}
            onChange={(val) => {
              setLocalStartTime(val as string);
              onStartTimeChange?.(val as string);
            }}
          />
        </div>
        ...
        <div className='flex'>
          ...
          <CustomSelect
            ...
            options={timeOptions}
            className='timePicker'
            value={localReturnTime || defaultTimeValue}
            onChange={(val) => {
              setLocalReturnTime(val as string);
              onReturnTimeChange?.(val as string);
            }}
          />
        </div>
      </div>
